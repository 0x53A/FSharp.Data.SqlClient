            ...
            fragment.Accept {
                new Microsoft.SqlServer.TransactSql.ScriptDom.TSqlFragmentVisitor() with
                    member __.Visit(node : Microsoft.SqlServer.TransactSql.ScriptDom.VariableReference) = 
                        base.Visit node
                        references.Add(node.Name, node.StartOffset, node.FragmentLength)
                    member __.Visit(node : Microsoft.SqlServer.TransactSql.ScriptDom.DeclareVariableElement) = 
                        base.Visit node
                        declarations.Add(node.VariableName.Value)
            }

            references, declarations, !errors

        let mutable tsql = cmd.Parameters.["@tsql"].Value.ToString()
        let references, declarations, parseErrors = getVariables tsql
        if parseErrors.Count <> 0 then None else
        let allVars =
            let refNames = references |> Seq.map (fun (name, _, _)  -> name)
            [ refNames ; declarations :> _ seq ] |> Seq.concat |> set
        let conflictingVars = 
            references
            |> Seq.toArray
            |> Seq.filter (fun (name, _, _) -> not (declarations.Contains name))
            |> Seq.groupBy (fun (name, _, _)  -> name)
            // get conflicts (same name referenced more than once)
            |> Seq.map (fun (k, g) -> k, (Seq.toArray g))
            |> Seq.filter (fun (k, g) -> g.Length >= 2)
            // skip the first occurence of each conflict (since a single usage is OK - we only rename subsequent usages)
            // ungroup so we have a flat list
            |> Seq.map (fun (_, g) -> g.[1..])
            |> Seq.collect id
            // sort it so the last reference is first
            |> Seq.sortBy (fun (_, start, _) -> Int32.MaxValue - start)
        let modifiedTsql, renamedVars =
            conflictingVars
            // replace each conflict with an autogenerated name that will be ignored later
            |> Seq.fold (fun text (name, start, length) -> text) (tsql, set [])
